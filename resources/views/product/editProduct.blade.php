            ...
            <div class="space-y-4 bg-white px-4 w-3/4">
                <form action="{{ route('product.update', $producto) }}" method="POST">
                    ...
                    <textarea name="descripcion" rows="5" class="border-gray-600/50 p-2 rounded-md outline-none border-[1px] w-full"
                        aria-label="descripcion">{{ old('descripcion', $producto->descripcion) }}</textarea>
                    @error('descripcion')
                        <span class="text-red-500 text-sm">{{ $message }}</span><br>
                    @enderror
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input name="precio" type="number" min="0" step="0.5"
                                ...
                                aria-label="precio" value="{{ old('precio', $producto->precio) }}">
                            @error('precio')
                                <span class="text-red-500 text-sm">{{ $message }}</span><br>
                            @enderror
                        </div>
                        <div>
                            ...
                            <select name="idcategoria"
                                ...
                                @foreach ($categories as $categoria)
                                    <option {{ $producto->idcategoria == $categoria->id ? 'selected' : '' }}
                        ...
                        <div>
                            ...
                            <input name="descuento" type="number" min="0" step="0.5"
                                class="border-gray-600/50 p-1 rounded-md outline-none border-[1px] w-full"
                                aria-label="descuento" value="{{ old('descuento', $producto->descuento) }}">
                            @error('descuento')
                                <span class="text-red-500 text-sm">{{ $message }}</span><br>
                            @enderror
                        </div>
                        <div>
                            ...
                            <select name="estadoProducto"
                                class="border-gray-600/50 p-1 rounded-md outline-none border-[1px] w-full">
                                <option {{ $producto->estadoProducto == 'nuevo' ? 'selected' : '' }} value="nuevo">Nuevo
                                </option>
                        ...
                        <div>
                            ...
                            <input name="stock" type="number" min="0" step="1"
                                class="border-gray-600/50 p-1 rounded-md outline-none border-[1px] w-full"
                                aria-label="stock" value="{{ old('stock', $producto->stock) }}">
                            @error('stock')
                                <span class="text-red-500 text-sm">{{ $message }}</span><br>
                            @enderror
                        </div>
