...
<div class="grid grid-cols-4 gap-4">
    ...
    <p class="text-gray-500 hover:text-gray-800"><a href="{{ route('searchcategory',['categoria'=>$category->id]) }}">{{$category->nombre}}</a></p>
     @endforeach
  ...
  <button type="submit" class="bg-blue-600 text-white rounded-lg p-2">></button>
    ...
    @foreach ($buscar as $item)
       ...
       <img src="{{$item->foto}}" width="40%" alt="" srcset="">
            ...
            <div class="p-5">
            <a href="{{ route('product.detail', $item->id)}}"> <h1 class="text-3xl">{{$item->nombre}}</h1></a>
